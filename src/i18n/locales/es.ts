export const es = {
  translation: {
    header: {
      title: "Estadísticas Pokémon"
    },
    buttons: {
      victory: "Victoria +",
      victoryRemove: "Victoria -",
      defeat: "Derrota +",
      defeatRemove: "Derrota -",
      reset: "Resetear Estadísticas"
    },
    stats: {
      statistics: "Estadísticas",
      victories: "Victorias",
      defeats: "Derrotas",
      winPercentage: "Porcentaje de Victoria",
      classification: "Clasificación",
      ranks: {
        normal: "Normal",
        ace: "Ace",
        veteran: "Veterano",
        expert: "Experto",
        legend: "Leyenda"
      }
    },
    nav: {
      home: "Inicio",
      teams: "Equipos"
    },
    confirmReset: "¿Estás seguro de que quieres resetear todas las estadísticas?",
    calendar: {
      title: "Registro de Juego",
      months: {
        january: "enero",
        february: "febrero",
        march: "marzo",
        april: "abril",
        may: "mayo",
        june: "junio",
        july: "julio",
        august: "agosto",
        september: "septiembre",
        october: "octubre",
        november: "noviembre",
        december: "diciembre"
      },
      weekDays: {
        mon: "LUN",
        tue: "MAR",
        wed: "MIÉ",
        thu: "JUE",
        fri: "VIE",
        sat: "SÁB",
        sun: "DOM"
      }
    },
    team: {
      title: "Mi Equipo Pokemon",
      addPokemon: "Añadir Pokemon",
      removePokemon: "Eliminar",
      searchPlaceholder: "Buscar Pokemon...",
      emptyTeam: "Tu equipo está vacío",
      maxTeamSize: "Tamaño máximo del equipo alcanzado"
    },
    gameLog: {
      title: "Registro de Juego",
      date: "Fecha",
      result: "Resultado",
      victory: "Victoria",
      defeat: "Derrota",
      noGames: "No hay juegos registrados",
      total: "Total de Juegos",
      todayGames: "Juegos de Hoy"
    },
    gameRegister: {
      title: "Registro de Juego",
      selectDate: "Seleccionar fecha",
      months: {
        june: "junio"
        // ...otros meses
      }
    },
    teamButtons: {
      newTeam: "Nuevo Equipo",
      saveTeam: "Guardar Equipo"
    },
    teamManager: {
      title: "Mis Equipos Pokemon",
      emptySlot: "Vacío",
      pokemonCount: "Pokemon",
      buttons: {
        select: "Seleccionar",
        delete: "Eliminar"
      },
      search: {
        title: "Seleccionar Pokemon",
        placeholder: "Buscar Pokemon...",
        accept: "Aceptar",
        cancel: "Cancelar"
      }
    }
  }
};